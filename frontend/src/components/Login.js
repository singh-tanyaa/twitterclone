  ...
  return (
    <div className='w-screen h-screen flex items-center justify-center'>
      <div className='flex items-center justify-evenly w-[80%]'>
        <div>
          <img className='ml-5 w-48 md:w-56' src="/twitlogo.png" alt="twitter-logo" />
        </div>
        <div className='md:ml-10'>
          <div className='my-5'>
            <h1 className='font-bold text-4xl md:text-6xl'>Happening now.</h1>
          </div>
          <h1 className='mt-4 mb-2 text-2xl md:text-3xl font-bold text-left'>{isLogin ? "Login" : "Signup"}</h1>
          <form onSubmit={submitHandler} className='flex flex-col w-full md:w-[55%]'>
            {!isLogin && (
              <>
            ...
            <h1>{isLogin ? "Don't have an account?" : "Already have an account?"} <span onClick={loginSignupHandler} className='font-bold text-blue-600 cursor-pointer'>{isLogin ? "Signup" : "Login"}</span></h1>
          </form>
        </div>
      </div>
    </div>
